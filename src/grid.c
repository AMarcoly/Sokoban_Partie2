...
void init_level(const char* file_path,Grid* grid){
	...
	grid -> nbr_de_cibles = number_goals;
	grid ->pts_cibles_recouverts=0;
	...
	while(fgets(line, 100, file) != NULL){
		...
		while(*buffer && *buffer != '\n'){
			CaseType ct; //lecture du caractere courant
			switch (*buffer){
				case '#': ct = WALL;
				break;
				case '$': ct = BOX;
			...
			grid->game_grid[current_row][current_column]=ct; //on copie dans la grille de jeu le caractere lue au bon emplacement
			current_column += 1; //on incremente le nombre de colonnes
			buffer += 1; //on avance d'un caractere sur la ligne
		}		
		current_row += 1;	//on incremente le nombre de colonnes
	}
...
void display (Grid* grid){
	...
	printf("pts cibles : %d\n pts cibles rec %d\n",grid->nbr_de_cibles,grid->pts_cibles_recouverts);
...
/**
 * @brief fonction verifiant si le nombre de points necessaire est recouvert pour afficher la fonction
 * 
 * @param grid 
 * @return int 
 */
int equal_point(Grid * grid){
	return grid->nbr_de_cibles==grid->pts_cibles_recouverts;
}
